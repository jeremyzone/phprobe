...
int main() {
  ...
  int ADS_address = 0x44;       // base address of our device on the I2C bus
  int I2CFile;
...
//  while ((buffer[0] & 0x80) == 0)     // readBuf[0] contains 8 MSBs of config register, AND with 10000000 to select bit$
//  {
...
while (1)
  ...
  printf("pH Reading %.2f \r", (float) ((val-44000.0)/(-6500.0)));      // Print the result to terminal
}               
  close(I2CFile);
